...
import rp.com.google.common.cache.Cache;
...
public class SoapUIServiceImpl implements SoapUIService {
	...
	private static final Cache<String, Maybe<String>> ITEMS = CacheBuilder.newBuilder().build();
	...
	public void startTestStep(TestStep testStep) {
		...
		testStep.setPropertyValue(ID, toStringId(rs));

	}

	public void finishTestStep(TestStepResult testStepContext) {
		Maybe<String> testId = fromStringId(testStepContext.getTestStep().getPropertyValue(ID));
	...
	private String toStringId(Maybe<String> id) {
		String tempID = UUID.randomUUID().toString();
		ITEMS.put(tempID, id);
		return tempID;
	}

	private Maybe<String> fromStringId(String tempId) {
		Maybe<String> id = ITEMS.getIfPresent(tempId);
