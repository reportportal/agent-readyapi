...
/**
 * Default implementation of {@link SoapUIService}
 *
 * @author Raman_Usik
 */
public class SoapUIService {
    ...
    @Inject
    @Named("resultLoggers")
    private List<ResultLogger<?>> resultLoggers;
    ...
    @Inject
    public SoapUIService(TestPropertyHolder contextProperties) {
        ...
        try {
            for (ResultLogger<?> resultLogger : resultLoggers) {
                if (resultLogger.supports(testStepContext)) {
                    final List<SaveLogRQ> rqs = resultLogger.buildLogs(testId, testStepContext);
                    if (null != rqs) {
                        for (SaveLogRQ slRQ : rqs)
